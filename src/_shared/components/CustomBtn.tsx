...
const CustomBtn:FC<CustomBtnProps> = ({image, isNested, isActive, bgColor, brdColor, content, size,activeColor, handleClick}) => {
  return (
    <button onClick={handleClick} className={` text-lightGray ${isActive ? activeColor : "bg-transparent "} hover:border-mainRed flex flex-row items-center  font-medium rounded-md text-xl ${isNested ? "cursor-not-allowed border-0  text-white" : " pl-2 pr-2 py-2 space-x-2 border-[1px] border-lightGray"}`}>
        {
            isNested ? (
                <>
